...
$(document).ready(function() {
    ...
    $('#tmhtmlcontent_topColumn').find('.tmhtmlcontent-item-1').addClass('col-xs-offset-2');
    var itemLi = $('#htmlcontent_top ul li');
    ...
    if ($x > 768) {
        checkScene = $('body #htmlcontent_top');

        if (checkScene.length) {
            scene = document.getElementById('htmlcontent_top');
...
function listTabsAnimate(element) {
	if (!isMobile && jQuery(element).length) {
		TweenMax.staggerFromTo(element, 0.4, {alpha: 0, ease:Power1.easeOut},{alpha: 1, ease:Power1.easeOut}, 0.2);
	}
}
