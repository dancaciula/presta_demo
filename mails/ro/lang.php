<?php

global $_LANGMAIL;
$_LANGMAIL = array();

$_LANGMAIL['Your guest account has been transformed into a customer account'] = 'Contul dumneavoastră de oaspete a fost transformat în unul de client';
$_LANGMAIL['The virtual product that you bought is available for download'] = 'Produsul virtual pe care l-ați cumpărat este disponibil pentru descărcare';
$_LANGMAIL['New voucher for your order %s'] = 'Cupon nou pentru comanda %s';
$_LANGMAIL['Order confirmation'] = 'Confirmare comandă';
$_LANGMAIL['Log: You have a new alert from your shop'] = 'Log: Aveti o alerta noua din magazin';
$_LANGMAIL['Fwd: Customer message'] = 'Fwd: Mesaj client';
$_LANGMAIL['An answer to your message is available #ct%1$s #tc%2$s'] = 'Un răspuns la mesajul tău este posibil la #ct%1$s #tc%2$s ';
$_LANGMAIL['Your new password'] = 'Noua dumneavoastra parola';
$_LANGMAIL['Package in transit'] = 'Pachet in tranzit';
$_LANGMAIL['New message regarding your order'] = 'Mesaj nou privind comanda dumneavoastră';
$_LANGMAIL['New credit slip regarding your order'] = 'Nota de credit noua pentru comanda';
$_LANGMAIL['New voucher for your order #%s'] = 'Cupon nou pentru comanda %s';
$_LANGMAIL['Process the payment of your order'] = 'Procesează plata comenzii dumneavoastră';
$_LANGMAIL['Your order return status has changed'] = 'Statusul returnării comenzii a fost modificat';
$_LANGMAIL['Welcome!'] = 'Bine ati venit!';
$_LANGMAIL['Your message has been correctly sent #ct%1$s #tc%2$s'] = 'Mesajul a fost trimis cu succes #ct%1$s #tc%2$s';
$_LANGMAIL['Your message has been correctly sent'] = 'Mesajul a fost trimis cu succes';
$_LANGMAIL['Message from contact form'] = 'Mesaj din formularul de contact';
$_LANGMAIL['Message from a customer'] = 'Mesaj de la un client';
$_LANGMAIL['Password query confirmation'] = 'Confirmare interogare parola';
$_LANGMAIL['Newsletter voucher'] = 'Cupon reducere newsletter';
$_LANGMAIL['Newsletter confirmation'] = 'Confirmare abonare';
$_LANGMAIL['Email verification'] = 'Verificare e-mail';
$_LANGMAIL['%1$s sent you a link to %2$s'] = '%1$s v-a trimis o legătură către %2$s ';


return $_LANGMAIL;
